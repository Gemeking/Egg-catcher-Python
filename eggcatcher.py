from itertools import cycle
from random import randrange
import requests
from io import BytesIO
from tkinter import Canvas, Tk, messagebox, PhotoImage, font
from PIL import Image, ImageTk

# Canvas and game settings
canvas_width = 800
canvas_height = 400
root = Tk()
root.title("Egg Catcher")

# Fetch and load images from URLs
def fetch_image(url):
    response = requests.get(url)
    image_data = BytesIO(response.content)
    return Image.open(image_data)

# Background images
sky_image_url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSrLyhQaHg45zMVe_qZ10azlmOxAiNyqgNA1w&s"
ground_image_url = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTTQ7zzFl_4GxIamMqyaIWqfhu4Cho2hOLwGg&s"

# Egg image
egg_image_url = "https://icons.iconarchive.com/icons/google/noto-emoji-food-drink/512/32390-egg-icon.png"

# Load and resize images
sky_image = fetch_image(sky_image_url).resize((canvas_width, canvas_height), Image.ANTIALIAS)
ground_image = fetch_image(ground_image_url).resize((canvas_width, 100), Image.ANTIALIAS)
egg_image = fetch_image(egg_image_url).resize((45, 55), Image.ANTIALIAS)

# Convert images to PhotoImage format
sky_photo = ImageTk.PhotoImage(sky_image)
ground_photo = ImageTk.PhotoImage(ground_image)
egg_photo = ImageTk.PhotoImage(egg_image)

# Create the canvas
c = Canvas(root, width=canvas_width, height=canvas_height)

# Add background images
c.create_image(0, 0, image=sky_photo, anchor="nw")
c.create_image(0, canvas_height - 100, image=ground_photo, anchor="nw")
c.pack()

# Game constants
egg_width = 45
egg_height = 55
egg_score = 10
egg_speed = 500
egg_interval = 4000
difficulty = 0.95
catcher_color = "blue"
catcher_width = 100
catcher_height = 100
catcher_startx = canvas_width / 2 - catcher_width / 2
catcher_starty = canvas_height - catcher_height - 20
catcher_startx2 = catcher_startx + catcher_width
catcher_starty2 = catcher_starty + catcher_height

# Catcher setup
catcher = c.create_arc(catcher_startx, catcher_starty, catcher_startx2, catcher_starty2, start=200, extent=140, style="arc", outline=catcher_color, width=3)

# Score and lives
game_font = font.nametofont("TkFixedFont")
game_font.config(size=18)
score = 0
score_text = c.create_text(10, 10, anchor="nw", font=game_font, fill="darkblue", text="Score: " + str(score))
lives_remaining = 3
lives_text = c.create_text(canvas_width - 10, 10, anchor="ne", font=game_font, fill="darkblue", text="Lives: " + str(lives_remaining))

# Egg list
eggs = []

def create_egg():
    x = randrange(10, 740)
    y = 40
    new_egg = c.create_image(x, y, anchor='nw', image=egg_photo)
    eggs.append(new_egg)
    root.after(egg_interval, create_egg)

def move_eggs():
    for egg in eggs:
        (eggx, eggy) = c.coords(egg)
        c.move(egg, 0, 10)
        if eggy + egg_height > canvas_height:
            egg_dropped(egg)
    root.after(egg_speed, move_eggs)

def egg_dropped(egg):
    eggs.remove(egg)
    c.delete(egg)
    lose_a_life()
    if lives_remaining == 0:
        messagebox.showinfo("Game Over!", "Final Score: " + str(score))
        root.destroy()

def lose_a_life():
    global lives_remaining
    lives_remaining -= 1
    c.itemconfigure(lives_text, text="Lives: " + str(lives_remaining))

def check_catch():
    (catcherx, catchery, catcherx2, catchery2) = c.coords(catcher)
    for egg in eggs:
        (eggx, eggy) = c.coords(egg)
        if catcherx < eggx < catcherx2 and catchery2 - eggy < 40:
            eggs.remove(egg)
            c.delete(egg)
            increase_score(egg_score)
    root.after(100, check_catch)

def increase_score(points):
    global score, egg_speed, egg_interval
    score += points
    egg_speed = int(egg_speed * difficulty)
    egg_interval = int(egg_interval * difficulty)
    c.itemconfigure(score_text, text="Score: " + str(score))

def move_left(event):
    (x1, y1, x2, y2) = c.coords(catcher)
    if x1 > 0:
        c.move(catcher, -20, 0)

def move_right(event):
    (x1, y1, x2, y2) = c.coords(catcher)
    if x2 < canvas_width:
        c.move(catcher, 20, 0)

c.bind("<Left>", move_left)
c.bind("<Right>", move_right)
c.focus_set()

root.after(1000, create_egg)
root.after(1000, move_eggs)
root.after(1000, check_catch)

root.mainloop()

